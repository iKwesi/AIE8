...
def _build_model_with_tools(rag_chain=None):
    """Return a chat model instance bound to the current tool belt."""
    model = get_openai_model(model_name="gpt-4o-mini")
...
def helpfulness_node(state: AgentState) -> Dict[str, Any]:
  ...
  {final_response}"""
    ...
    helpfulness_check_model = get_openai_model(model_name="gpt-4o-mini")
    helpfulness_chain = (
...
def build_graph(rag_chain=None):
    """Build an agent graph with an auxiliary helpfulness evaluation subgraph.
    
    Args:
        rag_chain: Optional ProductionRAGChain to include as a tool
        
    Returns:
        Compiled LangGraph with helpfulness checking
    """
